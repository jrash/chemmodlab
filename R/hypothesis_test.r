...
PerfCurveTest <- function(S1, S2, X, r, metric = "rec", method = "AH",
  ...
  if(plus2) {
    # Treat the plus2 hits as if they were the sames compounds for the
    # two scoring algorithms. If they are treated as different compounds,
    # there can be negative variances observed at early fractions.
    # This caused by performing the plus2 correction on prec and then transforming to rec
    pi1 <- (hits1+2)/(m*r+4)
    k1 <- r/pi.0*pi1
    pi2 <- (hits2+2)/(m*r+4)
    k2 <- r/pi.0*pi2
  ...
  if(metric == "rec") {
    if(method %in% c("JZ ind", "AH")){
      for(j in seq_along(k1)) {
        ...
        CI.int[j, ] <- c( (hits1-hits2)[j]/nact - quant*se[j],
                          (hits1-hits2)[j]/nact + quant*se[j])
        zscore <- ( (hits1-hits2)[j]/nact )/se[j]
